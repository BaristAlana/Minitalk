...
/*   Updated: 2024/02/15 00:07:30 by aherbin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sig_printf(int signum, siginfo_t *info, void *ucontent)
{
	...
	if (i == 8)
	{
		...
		bit = 0;
		usleep(50);
		if (kill(info->si_pid, SIGUSR2) == -1) //BYTE received, keep sending
			exit(0);
	}
	usleep(50);
	if (kill(info->si_pid, SIGUSR1) == -1) //bit received, keep sending
		exit(0);
}
